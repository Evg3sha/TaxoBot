...
def comparison(chat_id, user_price, from_long, from_lat, to_long, to_lat):
    ...
    ya_test = price_ya - 50 < user_price < price_ya + 50
    city_test = float(price_city) - 50 < user_price < float(price_city) + 50

    if city_test and ya_test:
        if float(price_city) < price_ya:
            mybot.send_message(chat_id, '[Цена в пределах допустимого диапазона. Перейдите в приложение Ситимобил](http://onelink.to/5m3naz)', parse_mode='Markdown')
        else:
            mybot.send_message(chat_id,
                               '[Цена в пределах допустимого диапазона. Перейдите в приложение Яндекс.Такси](https://3.redirect.appmetrica.yandex.com/route?utm_source=serp&utm_medium=org&start-lat={}&start-lon={}&end-lat={}&end-lon={}&ref=402d5282d269410b9468ae538389260b&appmetrica_tracking_id=1178268795219780156)', parse_mode='Markdown')
    else:
        if city_test:
            mybot.send_message(chat_id, '[Цена в пределах допустимого диапазона. Перейдите в приложение Ситимобил](http://onelink.to/5m3naz)', parse_mode='Markdown')
